...
import {
  List, Plus, Trash2, Edit2, Globe, Lock, Users, Eye, Pencil,
  X, ChevronRight, BookOpen, UserPlus, Shield, LogOut
} from "lucide-react";
...
export const ShareModal = ({ list, onClose, onListUpdated }) => {
  ...
  useEffect(() => {
    axios.get(`/api/lists/${list.id}/permissions`)
  ...
  const handleInvite = async () => {
    ...
    setLoading(true);
    try {
      const r = await axios.post(`/api/lists/${list.id}/permissions`, { email: inviteEmail.trim(), role: inviteRole });
      if (r.data.success) {
  ...
  const handleRoleChange = async (userId, newRole) => {
    try {
      await axios.put(`/api/lists/${list.id}/permissions/${userId}`, { role: newRole });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-[#1e2329] border border-white/10 rounded-2xl shadow-2xl w-full max-w-lg p-6 space-y-5" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-bold text-white flex items-center gap-2"><Users size={18} className="text-indigo-400"/>Share "{list.title}"</h2>
          <button onClick={onClose} className="text-neutral-400 hover:text-white"><X size={18}/></button>
        </div>

        {/* Visibility */}
        <div>
          <p className="text-xs text-neutral-400 mb-2 font-semibold uppercase tracking-wider">Visibility</p>
          <div className="flex gap-2">
            {["private","shared","public"].map(v => (
              <button key={v} onClick={() => handleVisChange(v)}
        ...
        {vis === "public" && (
          <div className="bg-[#272e36]/50 p-4 rounded-xl border border-white/5">
            <p className="text-xs text-neutral-400 mb-2 font-semibold uppercase tracking-wider">Public Link</p>
            <div className="flex gap-2">
              <input readOnly value={`${window.location.origin}/app/lists/${list.id}`}
        ...
        {/* Invite */}
        <div>
          <p className="text-xs text-neutral-400 mb-2 font-semibold uppercase tracking-wider">Invite People</p>
          <div className="flex gap-2">
        ...
        {permissions.length > 0 && (
          <div className="space-y-2">
            <p className="text-xs text-neutral-400 font-semibold uppercase tracking-wider">People with access</p>
            {permissions.map(p => (
              <div key={p.user_id} className="flex items-center justify-between bg-[#272e36] rounded-xl px-3 py-2">
                <div>
                  <p className="text-sm text-white font-medium">{p.user_name}</p>
                  <p className="text-xs text-neutral-500">{p.user_email}</p>
                </div>
                <div className="flex items-center gap-2">
                  <select value={p.role} onChange={e => handleRoleChange(p.user_id, e.target.value)}
                    className="rounded-lg border border-white/10 bg-[#1e2329] px-2 py-1 text-xs text-white outline-none">
                    <option value="viewer">Viewer</option>
                    <option value="editor">Editor</option>
                  </select>
                  <button onClick={() => handleRemove(p.user_id)} className="p-1 text-neutral-400 hover:text-red-400 transition-colors">
                    <X size={14}/>
                  </button>
                </div>
              </div>
...
const CardsModal = ({ list, canEdit, onClose }) => {
    ...
    const fetchData = async () => {
      try {
        const [lc, mc] = await Promise.all([
          ...
          canEdit ? axios.get("/api/card/cards", { params: { limit: 200 } }) : Promise.resolve({ data: { cards: [] } }),
        ]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-[#1e2329] border border-white/10 rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[80vh]" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center p-5 border-b border-white/5">
          <h2 className="text-lg font-bold text-white flex items-center gap-2"><BookOpen size={18} className="text-indigo-400"/>{list.title}</h2>
          <button onClick={onClose} className="text-neutral-400 hover:text-white"><X size={18}/></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {loading ? (
            ...
            listCards.length === 0 ? (
              ...
              <div key={c.card_id} className="bg-[#272e36] rounded-xl p-3 flex items-start justify-between gap-3 group">
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-white font-medium line-clamp-2">{c.question}</p>
                  {c.folder && <span className="text-xs text-neutral-500 mt-0.5 block">{c.folder}</span>}
                </div>
                {canEdit && (
                  <button onClick={() => handleRemove(c.card_id)}
            ...
            myCards.filter(c => !listCardIds.has(c._id ?? c.id)).map(c => (
              <div key={c._id ?? c.id} className="bg-[#272e36] rounded-xl p-3 flex items-start justify-between gap-3 group">
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-white font-medium line-clamp-2">{c.question}</p>
                  {c.folder && <span className="text-xs text-neutral-500 mt-0.5 block">{c.folder}</span>}
                </div>
                <button onClick={() => handleAdd(c._id ?? c.id)}
...
const ListFormModal = ({ existing, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (existing) {
        const r = await axios.put(`/api/lists/${existing.id}`, { title, description, visibility });
        if (r.data.success) { toast.success("List updated!"); onSave(); }
      } else {
        const r = await axios.post("/api/lists", { title, description, visibility });
        if (r.data.success) { toast.success("List created!"); onSave(); }
      }
    } catch (err) { toast.error(err.response?.data?.error ?? "Failed"); }
    setSaving(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-[#1e2329] border border-white/10 rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-bold text-white">{existing ? "Edit List" : "New List"}</h2>
          <button onClick={onClose} className="text-neutral-400 hover:text-white"><X size={18}/></button>
        </div>
        ...
        <div className="space-y-3">
          <div>
            ...
            <input value={title} onChange={e => setTitle(e.target.value)}
              ...
              className="w-full rounded-xl border border-white/10 bg-[#272e36] px-3 py-2.5 text-white placeholder-neutral-500 focus:border-indigo-500 outline-none text-sm" autoFocus/>
          </div>
          <div>
          ...
          <div>
            <label className="text-xs text-neutral-400 mb-1 block">Visibility</label>
            <div className="flex gap-2">
              {["private","shared","public"].map(v => (
        ...
        <div className="flex gap-2 justify-end pt-2">
          <button onClick={onClose} className="px-4 py-2 rounded-xl bg-[#272e36] text-neutral-300 hover:bg-[#2a3441] text-sm border border-white/5">
            Cancel
          </button>
    ...
    <div className="w-full max-w-4xl mx-auto p-2 md:p-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-400 mt-1 text-sm">Organise cards into shareable study lists</p>
        </div>
        <button onClick={() => { setEditingList(null); setShowForm(true); }}
      ...
      {loading ? (
        ...
        <>
          {/* My Lists */}
          <section>
            <h2 className="text-xs text-neutral-500 font-semibold uppercase tracking-wider mb-3">My Lists ({myLists.length})</h2>
            {myLists.length === 0 ? (
              ...
              <div className="grid gap-3">
                {myLists.map(l => (
                  <ListCard key={l.id} list={l} isOwner
                    onEdit={() => { setEditingList(l); setShowForm(true); }}
          ...
          {sharedWithMe.length > 0 && (
            <section>
              <h2 className="text-xs text-neutral-500 font-semibold uppercase tracking-wider mb-3">Shared with me ({sharedWithMe.length})</h2>
              <div className="grid gap-3">
                {sharedWithMe.map(l => (
      ...
      {/* Modals */}
      {showForm && (
        <ListFormModal existing={editingList} onClose={() => setShowForm(false)}
          onSave={() => { setShowForm(false); fetchLists(); }}/>
      )}
      {shareList && (
        <ShareModal list={shareList} onClose={() => setShareList(null)} onListUpdated={fetchLists}/>
      )}
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4"
          onClick={() => setDeleteTarget(null)}>
          <div className="bg-[#1e2329] border border-white/10 rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4"
            onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3">
              <Trash2 size={20} className="text-red-500"/>
              <h3 className="text-lg font-bold text-white">Delete List</h3>
            </div>
            <p className="text-neutral-300 text-sm">Delete <span className="font-semibold text-white">"{deleteTarget.title}"</span>? Cards won't be deleted.</p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setDeleteTarget(null)} className="px-4 py-2 rounded-xl bg-[#272e36] text-neutral-300 text-sm border border-white/5">Cancel</button>
              <button onClick={handleDelete} className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-500 text-white text-sm font-medium flex items-center gap-1.5">
  ...
  <div className="group bg-[#1e2329] border border-white/5 hover:border-white/10 rounded-2xl p-4 transition-all flex items-center gap-4">
    ...
    <div className="flex-1 min-w-0 cursor-pointer" onClick={onOpen}>
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-white font-semibold text-sm truncate">{list.title}</span>
        <VisibilityBadge visibility={list.visibility}/>
        <RoleBadge role={list.my_role}/>
      </div>
      {list.description && <p className="text-xs text-neutral-500 mt-0.5 truncate">{list.description}</p>}
      <div className="flex items-center gap-3 mt-1">
        <span className="text-xs text-neutral-500">{list.card_count ?? 0} cards</span>
        {!isOwner && <span className="text-xs text-neutral-600">by {list.owner_name}</span>}
      </div>
    </div>
    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
      <button onClick={onOpen} title="View cards"
