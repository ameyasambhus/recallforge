...
const Log = () => {
  ...
  const aiUsagePercent = aiAnswersLimit
    ? Math.min(100, Math.round((aiUsedThisMonth / aiAnswersLimit) * 100))
    : 0;
      ...
      <div className="w-full max-w-2xl rounded-2xl border border-white/10 bg-[#272e36] p-8 shadow-lg">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <div className="rounded-xl border border-white/10 bg-[#1c2229] p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm text-white">
                    You are on the <span className="font-semibold text-indigo-300">{PLAN_LABELS[activePlan]}</span> plan
                  </p>
                  <p className="text-xs text-neutral-400">
                    AI usage: {aiUsedThisMonth}/{aiAnswersLimit} this month
                  </p>
                </div>
                <Link
                  to="/app/settings"
                  className="inline-flex items-center justify-center rounded-lg border border-indigo-500/40 bg-indigo-500/10 px-3 py-2 text-xs font-semibold text-indigo-200 transition-all hover:border-indigo-400 hover:bg-indigo-500/20"
                >
                  View plans in Settings
                </Link>
              </div>
              <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-neutral-950/70">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-indigo-500 via-indigo-400 to-purple-500 transition-all"
