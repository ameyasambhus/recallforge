...
import { Role, Visibility } from '../models/listModel.js';
...
export const updateList = async (req: Request, res: Response) => {
  try {
    const { title, description, visibility } = req.body;
    const list = await listService.updateList(req.params.id, req.user.id, { title, description, visibility });
...
export const deleteList = async (req: Request, res: Response) => {
  try {
    const deleted = await listService.deleteList(req.params.id, req.user.id);
...
export const getListCards = async (req: Request, res: Response) => {
  try {
    const cards = await listService.getListCards(req.params.id, req.user.id);
...
export const addCardToList = async (req: Request, res: Response) => {
  try {
    ...
    await listService.addCardToList(req.params.id, cardId);
...
export const removeCardFromList = async (req: Request, res: Response) => {
  try {
    ...
    await listService.removeCardFromList(req.params.id, req.params.cardId);
...
export const copyCardFromList = async (req: Request, res: Response) => {
  try {
    ...
    const result = await listService.copyCardFromList(req.params.cardId, req.user.id);
    if (result.success) {
...
export const getPermissions = async (req: Request, res: Response) => {
  try {
    ...
    const permissions = await listService.getPermissions(req.params.id);
...
export const inviteUser = async (req: Request, res: Response) => {
  try {
    ...
    const result = await listService.inviteUser(req.params.id, email, role as Role);
    if (!result.success) return res.status(404).json({ error: result.message });

    // Update visibility to 'shared' if it was private
    if (list.visibility === 'private') {
      await listService.updateList(req.params.id, req.user.id, { visibility: 'shared' });
    }
...
export const removePermission = async (req: Request, res: Response) => {
  try {
    ...
    await listService.removePermission(req.params.id, req.params.userId);
...
export const updatePermission = async (req: Request, res: Response) => {
  try {
    ...
    await listService.updatePermission(req.params.id, req.params.userId, role as Role);
